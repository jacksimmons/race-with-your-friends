...
func start_Pre_Config() -> void:
	if !local_pre_config_done:
		# Load Scene
		var scene = load("res://Scene/Scene.tscn").instance()
		get_node("/root").add_child(scene)
		
		# Load the Map
		map = load("res://Scene/Map/Scorpion/ScorpionMap.tscn").instance()
		...
		for player_id in ids:
			if int(player_id) != SteamGlobals.STEAM_ID:
				...
				var player = load("res://Scene/Vehicle/" + vehicle + ".tscn").instance()
				player.set_name(str(player_id))
				players.add_child(player)
				
				var cam = preload("res://Scene/Cam.tscn").instance()
...
func start_Bot_Config() -> void:
	for i in range(SteamGlobals.BOT_DATA.size()):
		...
		var bot = load("res://Scene/Vehicle/" + vehicle + ".tscn").instance()
